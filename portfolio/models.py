...
class PortfolioIndexPage(Page):
    ...
    hero_title = StreamField([('title', blocks.CharBlock())],
...
class PortfolioPage(Page):
    ...
    industries = ParentalManyToManyField('portfolio.PortfolioIndustryCategory', blank = True)
    technologies = ParentalManyToManyField('portfolio.PortfolioTechnologyCategory', blank = True)
    ...
    def get_recent_projects(self):
            max_count = 1
            return PortfolioPage.objects.all().order_by(
                '-title')[:max_count]
...
class Gallery(Orderable):
    ...
    page = ParentalKey(PortfolioPage, on_delete=models.CASCADE, related_name='portfolio_gallery')
...
# Class for to show book marked projects 
